...
function buildFrontmatter(data) {
    ...
    if (data.url)
        lines.push('url: ' + yamlEscape(data.url));
    if (data.image)
        lines.push('image: ' + yamlEscape(data.image));
    if (lines.length === 0)
        return '';
    return '---\n' + lines.join('\n') + '\n---';
}
        ...
        if (isTag(child)) {
            ...
            if (tag === 'header') {
                const isInsideSectioning = node && isTag(node) &&
                    SECTIONING_TAGS.has(node.tagName.toLowerCase());
...
function isUsefulMeta(el) {
    ...
    // Useful meta names
    if (['description', 'author', 'keywords', 'robots', 'viewport'].includes(name))
        return true;
    // Open Graph
...
function isHidden(el, options) {
    if (el.attribs.hidden !== undefined)
        return true;
    if (!options.keepAriaHidden && el.attribs['aria-hidden'] === 'true')
        return true;
...
function hasContentAttributes(el, options = {}) {
    for (const key of Object.keys(el.attribs)) {
        ...
        if (CONTENT_ATTRIBUTES.has(lowerKey))
            return true;
        if (lowerKey === 'id' && options.keepIds !== false)
            return true;
        if (lowerKey === 'class' && options.keepClasses)
            return true;
        if (isDataAttribute(lowerKey) && options.keepDataAttributes)
            return true;
    }
    return false;
}
...
function markdownRewrite(node) {
    if (!hasChildren(node) && !isTag(node))
        return;
 ...
 */
export function wwwaxe(html, options = {}) {
    const markdown = options.markdown !== false;
